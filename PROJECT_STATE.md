...
- **Exactly-one-visible is enforced in Rust, not trusted from JS.** Both `browser_show_tab` and `browser_create_tab` call `hide_all_browser_webviews_except` (covers Tauri's registry *and* the persistent handle map) before showing. `show` originally lacked it and trusted the frontend to have hidden the outgoing tab — any missed or out-of-order hide left the old page rendered over the new one ("tab switches but webview doesn't"). Keep this call in every path that makes a tab visible. On the JS side the switch effect carries a sequence guard (`switchSeqRef`): a superseded async show/create must not apply its result, and reconciles via `ensureWebviewVisible()` instead. The null-bounds case retries (8×50ms) rather than returning — bailing stranded the switch permanently, since the effect's deps don't change again (this was the minimize→restore→switch repro).
...
- Viewport Panel: **one device at a time at 1:1**, under one reused native webview (`viewport-active`); the sidebar list is the picker, tablets are added landscape, no zoom control. The webview is built exactly like a tab — `about:blank` → configure → `navigate()` — because the device user agent is a *build-time* `WebviewBuilder` attribute. URL change calls `navigate_viewport`; device change rebuilds (new UA) and so reloads the page; `on_page_load(Finished)` re-applies emulation and emits `viewport://loaded`. CDP carries only what geometry can't: DPR, `mobile`, touch.
  - **The frame is 1:1 and never scaled.** It renders at the device's real CSS pixels, clamped to the panel by CSS (`maxWidth/maxHeight: 100%`); the page scrolls inside it like a phone with a shorter screen. The measured card rect is the single source of truth for **both** the webview bounds and the CDP layout viewport — they are the same number, so they cannot drift. Do not reintroduce a display scale: five separate bugs came from needing bounds, WebView2 zoom factor and the CDP override to agree across racing async calls. Symptoms were a frame overflowing its card by exactly `1/scale` (zoom lost) and a page laid out wider than the surface showing it, i.e. cropped and off-centre (override applied, zoom lost). `set_zoom` is not used on this path at all.
  - **Gotcha:** the frame is a `Window::add_child` webview that clips to the *window*, not to the panel, so a frame larger than the panel draws over the toolbar and sidebar. CSS clamping is what makes that structurally impossible — keep `overflow-hidden` on the surface and the `max-*: 100%` on the card.
  - **Gotcha:** never configure the webview *after* pointing it at the real URL, and never destroy+recreate on navigation. The UA is a build-time `WebviewBuilder` attribute, so configuring late means the first document request goes out with the desktop UA and server-side mobile detection never fires.
  - `probe_viewport` reports the page's real `innerWidth/innerHeight/devicePixelRatio/maxTouchPoints/userAgent` into the card header, via `eval_json` — Rust→page `ExecuteScript`, which works for remote content where page→Rust IPC does not. Added because every previous fix was diagnosed by inferring native state from screenshots; it caught two real bugs on its first run. Compared against the size last *sent* (not a DOM rect measured during render, which races layout), and re-taken on both page load and frame resize — probing only on load reported the load-time size forever and showed a false mismatch after any window resize.
  - **Gotcha:** the card needs an explicit `width: device.width` and `boxSizing: content-box`. Without the width it shrink-wraps its widest child — the header — and the body stretches past the device (the header's own probe text inflated the frame to 422 and that wrong width went to CDP). Without `content-box`, Tailwind's global `border-box` lets the 1px border eat into the device width and the frame comes out 2px narrow.
  - **Gotcha:** the build-time UA sets the header and `navigator.userAgent` but NOT `Sec-CH-UA*`, and Chromium prefers Client Hints — so sites still saw the real Edge-on-Windows. `Emulation.setUserAgentOverride` must also carry `userAgentMetadata` (platform + `mobile` + brands).
  - **Gotcha:** `Emulation.setPageScaleFactor: 1` is required. With `mobile: true` Chromium shrink-to-fits whenever anything overflows the viewport (one ad banner is enough) — the layout viewport stayed a correct 412 but the page rendered zoomed out at exactly 2/3 in both axes. Real phones with `initial-scale=1` and Chrome's device toolbar at 100% both stay at scale 1 and let the overflow scroll.
  - **Gotcha:** check emulation against `documentElement.clientWidth/clientHeight` (the **layout** viewport), never `innerWidth/innerHeight` — those are the *visual* viewport and move with page scale, so they flagged a correct 412 layout as wrong. The probe reports both; a divergence between them means a page scale is in play.
  - **Gotcha:** bounds and emulation are one command (`resize_viewport` takes the `DeviceSpec` and re-asserts the override after `set_bounds`). As two separate invokes nothing ordered them, and a WebView2 controller resize resets the emulated viewport to the widget's natural size while *keeping* `deviceScaleFactor` — so whenever the resize landed last the page reported the raw widget size (422×726 for a 412×707 frame) with DPR still correctly 3.5. Never cache this on a spec signature: it must be re-asserted after every bounds change even when the spec is identical.
...
- Page→app IPC does not exist for remote content: `capabilities/default.json` declares no `remote` scope, so Tauri never injects `__TAURI_INTERNALS__` into `https://` webviews and any injected script calling `invoke` is a silent no-op. The viewport sync/metrics feature was built on that and has been deleted. Do not add `remote.urls` to revive this pattern — it exposes tab-scoped commands to any page; poll from Rust via `ExecuteScript` like the Inspector instead.
