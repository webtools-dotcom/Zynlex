...
- Viewport Panel: device presets in two non-scrolling fit-to-surface modes — `viewportLayout: "focus"` (default, selected device only, ~90% for a phone) and `"overview"` (all in one row, ~44%, for breakpoint scanning); tablets are added landscape. Device emulation splits two jobs: CDP `Emulation.setDeviceMetricsOverride`/`setTouchEmulationEnabled`/`setUserAgentOverride` for device characteristics, and `set_zoom` for the on-screen scale (`emulate_viewport`, same CDP plumbing as hard-reload/screenshot).
  - **Gotcha:** frames are `Window::add_child` webviews that clip to the *window*, not to the panel — hence `overflow-hidden`, zoom capped at fit (`Math.min(chosen, fit)`), and an explicit `hideViewport` for any viewport with no rendered card (focus mode renders one; the old `if (!node) continue` left the rest painted on top). Card chrome is measured, not a constant — compact mode changes the header height.
  - **Gotcha:** `fitZoom` takes the *whole* device list, never just the visible one — scaling each device to fill the panel individually made every phone render ~700px tall, so switching devices looked like nothing changed. Gaps are fixed px and don't scale, so they come off `availW` rather than being divided into it. The selection accent is overview-only; in focus the lone card is always selected, which boxed every device in a bright ivory ring.
  - **Gotcha:** zoom alone is not device emulation (it leaves monitor DPR, no touch, desktop UA — presets carried `deviceScaleFactor`/`mobile`/`touch` unused for exactly that reason), but CDP's `scale` field is not display scaling either: it applies to the "resulting view image", and a live WebView2 controller ignores it, so the page rendered at full width and overflowed its card by exactly `1/scale`. Keep both — CDP for metrics, `set_zoom` for display. They agree, since `bounds / zoom` = `preset`, the same width the override pins. Never pass `scale` to `setDeviceMetricsOverride`. **Gotcha:** a viewport with no rendered card must be explicitly hidden in `syncNativeViewports` — focus mode renders one card, and the old `if (!node) continue` left every other native webview painted on top; the sync's deps also carry a `visibleKey`, since switching between two same-size devices changes neither `viewports` nor the zoom.
...
- **Viewport scroll/click/input sync and the metrics badge are dead on real sites.** Their injected scripts call `window.__TAURI_INTERNALS__.invoke` from the page, and `capabilities/default.json` declares no `remote` scope, so Tauri never injects IPC into `https://` child webviews — the scripts' own `__TAURI_INTERNALS__` guards make them silent no-ops. Dead as a result: the three toolbar sync toggles, `getMetricsBadge`/`viewport://metrics`, `notify_viewport_*`, `scroll_viewport`/`click_viewport`, `useViewportSync`. Adding `remote.urls` is NOT the fix (it exposes tab-scoped commands to any page — see the architecture note); a real fix polls from Rust via `ExecuteScript` like the Inspector does. Decide: rewrite natively or delete.
