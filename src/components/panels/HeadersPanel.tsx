    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="h-8 flex items-center justify-between px-2 border-b border-[var(--color-border-subtle)] flex-shrink-0">
          <span className="text-[12px] font-semibold uppercase tracking-widest text-[var(--color-text-muted)]">
            Header Injection
          </span>
          <button
            onClick={() => setShowForm(true)}
            className="text-[12px] text-[var(--color-accent)] hover:text-[var(--color-text-primary)] cursor-pointer transition-colors"
          >
            + Add Rule
          </button>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="text-center">
            ...
            <p className="text-[12px] text-[var(--color-text-disabled)] mt-1 max-w-[180px] mx-auto">
    ...
    <div className="flex flex-col h-full">
      ...
      {showForm && (
        <div className="border-t border-[var(--color-border-subtle)] p-2 space-y-2">
          {/* URL Pattern */}
          <div>
            <label className="text-[12px] uppercase tracking-widest text-[var(--color-text-muted)] block mb-1">
              URL Pattern
            </label>
            <input
              type="text"
              ...
              className="w-full h-8 px-2 text-[13px] rounded border bg-[var(--color-surface)] border-[var(--color-border-subtle)] text-[var(--color-text-primary)]"
            />
            <p className="text-[12px] text-[var(--color-text-disabled)] italic mt-0.5">
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {QUICK_PICKS.map((pick) => (
                <button
                  ...
                  className="border rounded px-1.5 py-0.5 text-[12px] cursor-pointer hover:bg-[var(--color-hover)] text-[var(--color-text-muted)] border-[var(--color-border-subtle)] transition-colors"
                >
                  {pick}
                </button>
              ))}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-2 py-1 text-[13px] font-mono rounded border bg-[var(--color-surface)] border-[var(--color-border-subtle)] text-[var(--color-text-primary)] resize-none"
            />
            <p className="text-[12px] italic text-[var(--color-text-disabled)] mt-0.5">
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button
              ...
              className="text-[13px] text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] cursor-pointer transition-colors px-2 py-1"
            >
              Cancel
            </button>
            <button
              ...
              className="text-[13px] px-3 py-1 rounded bg-[var(--color-accent)] text-white disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer transition-opacity"
            >
              Add Rule
            </button>
          </div>
