...
export function validateMetaTags(meta: Record<string, string>): MetaValidation[] {
  ...
  if (!meta["og:title"]) {
    ...
    validations.push({ field: "og:title", value: meta["og:title"], status: "valid", message: "og:title present" });
  }

  if (!meta["og:description"]) {
    validations.push({ field: "og:description", value: "", status: "error", message: "Missing og:description" });
  } else {
    validations.push({ field: "og:description", value: meta["og:description"], status: "valid", message: "og:description present" });
  }
  ...
  if (!meta["og:image"]) {
    ...
    try {
      ...
      validations.push({ field: "og:image", value: meta["og:image"], status: "valid", message: "og:image present (absolute URL)" });
    } catch {
      validations.push({ field: "og:image", value: meta["og:image"], status: "warning", message: "og:image is relative URL — may fail for crawlers" });
    }
  }
  ...
  if (!meta["twitter:card"]) {
    ...
    validations.push({ field: "twitter:card", value: meta["twitter:card"], status: "warning", message: "Consider using summary_large_image for large image cards" });
  } else {
    validations.push({ field: "twitter:card", value: meta["twitter:card"], status: "valid", message: "twitter:card is summary_large_image" });
  }
