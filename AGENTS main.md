...
## 0. PROJECT MEMORY FILES — HOW TO USE THE 3 MD NOTES

These files are the agent’s external memory. Read them before making changes, and keep them current after each task.

### `PROJECT_STATE.md`
Use this for the live snapshot of the project:
- what is already finished
- what is in progress right now
- what is broken or blocked
- the latest decisions and known caveats

Keep it short, factual, and current. Update it after every meaningful task so the next session can resume fast.

### `ARCHITECTURE.md`
Use this for the stable map of the project:
- folder structure
- major components and boundaries
- data flow / control flow
- core conventions and design decisions
- important files and why they matter

Only update this file when the structure or a real design decision changes. Do not fill it with task-by-task noise.

### `TASKS.md`
Use this for the backlog:
- next tasks in priority order
- small follow-up items
- open bugs, improvements, and future work
- status markers like `todo`, `doing`, `blocked`, `done`

Keep tasks clear and bite-sized. Mark completed items instead of deleting them unless the file is meant to stay as a pure “next up” list.

### Working rule for all three
At the start of a task, check `PROJECT_STATE.md` first, then `ARCHITECTURE.md`, then `TASKS.md`, and only read extra source files that are directly needed. After the task, update the memory files that changed.
